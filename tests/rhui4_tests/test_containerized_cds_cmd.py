...
def test_03_add_safe_unknown_key():
    ...
    # try adding the CDS
    status = RHUIManagerCLIInstance.add(RHUA, "cds", cds, container=True)
...
def test_05_add_cds():
    ...
    for cds in CDS_HOSTNAMES:
        status = RHUIManagerCLIInstance.add(RHUA, "cds", cds, container=True, unsafe=True)
...
def test_10_readd_cds():
    ...
    # again choose a random CDS hostname from the list
    cds = random.choice(CDS_HOSTNAMES)
    status = RHUIManagerCLIInstance.add(RHUA, "cds", cds, container=True, force=True, unsafe=True)
...
def test_16_add_bad_cds():
    ...
    status = RHUIManagerCLIInstance.add(RHUA, "cds", "foo" + CDS_HOSTNAMES[0], container=True)
...
def test_17_delete_bad_cds():
    ...
    # so, add a node first
    cds = random.choice(CDS_HOSTNAMES)
    RHUIManagerCLIInstance.add(RHUA, "cds", cds, container=True, unsafe=True)
    cds_list = RHUIManagerCLIInstance.list(RHUA, "cds")
    nose.tools.eq_(cds_list, [cds])
    # deleting now
...
def test_18_add_cds_changed_case():
    ...
    status = RHUIManagerCLIInstance.add(RHUA, "cds", cds_up, container=True, unsafe=True)
...
def test_19_delete_unreachable():
    ...
    # the node remains configured (the container)... unconfigure it properly
    # do so by adding and deleting it again
    RHUIManagerCLIInstance.add(RHUA, "cds", cds, container=True, unsafe=True)
    RHUIManagerCLIInstance.delete(RHUA, "cds", [cds], force=True)

def test_20_check_cleanup():
    """check if rhui_cds was stopped and the remote file system unmounted on all CDSs """
    # for RHBZ#1640002
    service = "rhui_cds"
    dirty_hosts = {}
    errors = []

    dirty_hosts[service] = [cds.hostname for cds in CDS if Helpers.check_service(cds, service)]
    dirty_hosts["mount"] = [cds.hostname for cds in CDS if Helpers.check_mountpoint(cds, RHUI_ROOT)]

    if dirty_hosts[service]:
        errors.append(f"{service} is still running on {dirty_hosts[service]}")
